...
class ImportMusicWizard(models.TransientModel):
    ...
    def make_artist(self, artist_root, group):
        ...
        for i in artist_root:
            ...
            if i.tag == "singles":
                self.make_singles(i, artist=artist)
            if i.tag == "albums":
                self.make_albums(i, artist=artist)
    ...
    def make_group(self, group_root):
        ...
        for i in group_root:
            ...
            if i.tag == "artists":
                self.parse_artists(i, group=group)
            if i.tag == "albums":
                self.make_albums(i, group=group)
            if i.tag == "singles":
                self.make_singles(i, group=group)
